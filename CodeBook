...
After build these two dataframes, testDF and trainDF, a dataframe with the merged data of test and train was created with the
...
The objective of the x DF is to calculate the mean and the standard deviation of each row of this dataframe, which means to

calculate the mean and standard deviation of each measurement of the experiment.
...
ddply(mTestTrain,~subject_id~activity_name,summarise,mean=mean(measurements_means),mean_SD=mean(measurements_SD))
